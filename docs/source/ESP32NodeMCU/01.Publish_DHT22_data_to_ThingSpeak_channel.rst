...
ESP32 NodeMCU micro-controller will connect with DHT22 sensor, collects temperature and humidity reading, using WiFi publishes payload to MQTT server running on Raspberry Pi 3 and also publish payload to ThingSpeak server and readings can be visible on ThingSpeak public channel. Local MQTT server on Raspberry Pi 3 is used for test/debug purpose only. 
...
::
    ...
    */
    void connect_WiFi_Network(){
     ...
     /*   
      *    Debugging purpose to which WiFi network board is connected
